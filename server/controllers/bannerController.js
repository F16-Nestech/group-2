...
const bannerController = {
  validateBanner: async (req, res) => {
    ...
    if (!title || !imageUrl) {
      return res.status(400).json({
        message: "Vui lòng điền đầy đủ thông tin banner",
      });
    }
  },
  ...
  createBanner: async (req, res) => {
    try {
      ...
      if (existingBannerByTitle) {
        return res.status(400).json({
          message: "Banner đã tồn tại dựa trên tiêu đề",
        });
      }
  ...
  deleteBanner: async (req, res) => {
    try {
      ...
      const softDeleteBanner = await Banner.findByIdAndUpdate(bannerId, {
        deleted: true,
      });
  ...
  deleteManyBanner: async (req, res) => {
    ...
    try {
      ...
      const result = await Banner.deleteMany({ _id: { $in: ids } }).session(
        session
      );
